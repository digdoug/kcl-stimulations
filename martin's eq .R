...
cmb <- dt.scalz[dt.tauz[dt.betaz[dt.tz]]]
cmb2 <- dt.scalz[dt.tauz[dt.betaz[dt.tz]]]

# data values can't be treated as one possible combination

cmb[, data := data$mean[match(tz,data$timepoints)]]
cmb2[, data := data2$mean[match(tz,data2$timepoints)]]
cmb[, param_max := (scalz**2 + tauz**2 + betaz**2)]
cmb2[, param_max := (scalz**2 + tauz**2 + betaz**2)]
cmb[, param_min := ((1/scalz)**2 + (1/tauz)**2 + (1/betaz)**2)]
cmb2[, param_min := ((1/scalz)**2 + (1/tauz)**2 + (1/betaz)**2)]
 
  
cmb[,u := ifelse(tz > up,up,tz)] # up is when stimulus is taken off
cmb2[,u := ifelse(tz > up2,up2,tz)]
cmb[,value := mapply(function(...){integrate(...)$value},lower=0,upper=u,scale=scalz,concentrate=con,time=tz,lag_time=tauz,degrade=betaz,mrna=data,MoreArgs=list(f=Vectorize(function(x,scale,concentrate,time,lag_time,degrade,mrna){scale * (concentrate * (time - lag_time) - degrade * mrna)})))] # 7.9 
...
cmb2[,value := mapply(function(...){integrate(...)$value},lower=0,upper=u,scale=scalz,concentrate=con,time=tz,lag_time=tauz,degrade=betaz,mrna=data,MoreArgs=list(f=Vectorize(function(x,scale,concentrate,time,lag_time,degrade,mrna){scale * (concentrate * (time - lag_time) - degrade * mrna)})))] # 7.9 
...
train$value <- train$value + 1 # the single fastest MOST efficient step you'll ever take to account for background
train <- train[order(train$scalz,train$tauz,train$betaz),]

faction <- train[train$tauz != 0,] # exclude where parameters equal 0 (only a possibility for tauz in this case); results in Inf value for where parameter^-1 == 0 

param_max <- train[seq(from=1, to=nrow(train), by=length(tz)*2),7]
param_min <- faction[seq(from=1, to=nrow(faction), by=length(tz)*2),8]



first <- train[seq(from=1, to=nrow(train), by=length(tz)*2),2:4][which.min(colSums((genez$mean - matrix(train$value, nrow=length(tz)*2))**2)),] # minimized least squares
second <- faction[seq(from=1, to=nrow(faction), by=length(tz)*2),2:4][which.min(colSums((genez$mean - matrix(faction$value, nrow=length(tz)*2))**2) + param_min),] # minimized least squares

first}

good <- train[train$scalz == first$scalz & train$tauz == first$tauz & train$betaz == first$betaz,]
good1 <- train[train$scalz == second$scalz & train&tauz == second$tauz & train$betaz == second$betaz,]

good1
}


length(colSums((genes$mean - matrix(save$value, nrow=length(timez)*2))**2))

matrix(yo$value, nrow=length(timez)*2)
